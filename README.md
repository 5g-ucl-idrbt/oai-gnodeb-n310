# oai-gnodeb-310
1. Building and Installing the USRP Open-Source Toolchain (UHD) on Linux
Link: https://kb.ettus.com/Building_and_Installing_the_USRP_Open-Source_Toolchain_(UHD_and_GNU_Radio)_on_Linux

1.1. Update and Install dependencies.
$  sudo apt-get update

1.2. Once the system has been updated, then install the required dependencies for UHD and GNU Radio. 
...
1.3. Building and installing UHD from source code.
...
cd $HOME
   mkdir workarea
   cd workarea
1.4. Next, clone the repository and change into the cloned directory. 
$ git clone https://github.com/EttusResearch/uhd
 $ cd uhd
1.5. Next, checkout the desired UHD version. You can get a full listing of tagged releases by running the command: 
$  git tag -l
Outpu:
$ git tag -l
release_003_009_004
...
...
Note:- scroll down and select  version 4.4.0.0
1.6. After identifying the version and corresponding release tag you need, check it out: 
# Example: For UHD 4.4.0.0
  $ git checkout v4.4.0.0
1.7. Next, create a build folder within the repository. 
 $ cd host
 mkdir build
   cd build
1.8. Next, invoke Cmake.
$ cmake ..
1.9. Once the cmake command succeeds without errors, build UHD. 
$ make
1.10. Next, you can optionally run some basic tests to verify that the build process completed properly.
$  make test
1.11. Next, install UHD, using the default install prefix, which will install UHD under the /usr/local/lib folder. You need to run this as root due to the permissions on that folder. 
$ sudo make install
1.12. Next, update the system's shared library cache. 
$ sudo ldconfig
1.13. At this point, UHD should be installed and ready to use. You can quickly test this, with no USRP device attached, by running “uhd_find_devices”. You should see something similar to the following. 
